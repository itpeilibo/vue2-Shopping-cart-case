    ...
    dragSort () {
      ...
      const sortable = new Sortable(el, {
        ...
        onEnd: evt => {
          ...
          const arr = [...this.tableData] // 创建表数据的副本
          const draggedItem = arr.splice(evt.oldIndex, 1)[0] // 移除被拖拽的数据
          arr.splice(evt.newIndex, 0, draggedItem) // 插入被拖拽的数据到新位置
          this.tableData = arr // 更新表数据
          console.log('拖拽后', arr)
          // this.updateSort() 更新数据库，将排序后的数据传给后端
        }
        }
      ...
      const sortable1 = new Sortable(ee, {
        ...
        onEnd: evt => {
         if (evt.newIndex !== evt.oldIndex) {
          const arr = [...this.tableData2] // 创建表数据的副本
          const draggedItem = arr.splice(evt.oldIndex, 1)[0] // 移除被拖拽的数据
          arr.splice(evt.newIndex, 0, draggedItem) // 插入被拖拽的数据到新位置
          this.tableData2 = arr // 更新表数据
          console.log('拖拽后', arr)
          // this.updateSort() 更新数据库，将排序后的数据传给后端
        }
